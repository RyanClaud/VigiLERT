  ...
  <div class="min-h-screen flex flex-col bg-[#EDE8F5]">
    <div class="flex flex-col md:flex-row items-center justify-between py-4 px-4">
      <div class="flex items-center gap-2 bg-[#7091E6] px-6 py-2 rounded-xl shadow text-white">
        <span class="font-semibold text-lg">Emergency Contact Dashboard (For Viewing-Access Only)</span>
      </div>
      <button
        @click="logout"
    ...
    <main class="flex-1 px-4 md:px-8 py-6">
      ...
      <div v-else>
        ...
        <!-- Use LocationSection instead of static content -->
        <LocationSection
          :location="{
            lat: parseFloat(helmetData.locationLat),
            lng: parseFloat(helmetData.locationLng)
          }"
          :user="{ name: helmetData.userName || 'Rider' }"
        />
        ...
        <div class="bg-white rounded-lg shadow p-4 md:p-6 mb-4">
          ...
          <div class="text-xs text-[#8697C4] mb-2">Speed Limit: {{ helmetData.speedLimit || 10 }} kph</div>
        </div>
...
onMounted(() => {
  ...
  // Fetch helmet data (including user name)
  const helmetRef = dbRef(db, `helmet_public/${userId}`);
