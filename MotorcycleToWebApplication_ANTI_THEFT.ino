/* 
 * VigiLERT ESP32 Motorcycle Module - WITH ANTI-THEFT
 * ✅ All previous features
 * ✅ NEW: Anti-theft vibration detection when engine OFF
...
// ✅ NEW: Owner phone number for SMS alerts
...
// GSM module pins (UART2) - RE-ENABLED FOR SMS
#define GSM_RX 4
#define GSM_TX 5
...
// ✅ NEW: Anti-theft system variables
bool antiTheftEnabled = false;  // Automatically enabled when engine OFF
bool antiTheftArmed = false;    // Armed after 30 second delay
unsigned long engineOffTime = 0;
const unsigned long ARM_DELAY = 30000;  // 30 seconds to arm after engine off
const float THEFT_VIBRATION_THRESHOLD = 2.0;  // Lower threshold for theft detection
...
const int THEFT_DETECTION_REQUIRED = 3;  // Require 3 detections to avoid false alarms
...
void setup() {
  ...
  Serial.println("\n[SETUP] Starting VigiLERT System - WITH ANTI-THEFT...");
  ...
  Serial.println("[SETUP] Will activate 30s after engine stops");
...
void loop() {
  ...
  // ✅ ANTI-THEFT SYSTEM - Only active when engine OFF
  if (!engineRunning) {
    handleAntiTheft();
  } else {
  ...
  if (millis() - lastPrint > 5000) {
    Serial.printf("[STATUS] Engine:%s | Relay:%s | Alcohol:%s | AntiTheft:%s\n",
...
// ✅ NEW: Anti-theft handler
void handleAntiTheft() {
  ...
  if (!antiTheftArmed && (millis() - engineOffTime >= ARM_DELAY)) {
    ...
    theftDetectionCount = 0;
    theftAlertSent = false;
    Serial.println("\n[ANTI-THEFT] 🛡️ ARMED! Monitoring for unauthorized movement...");
  ...
  // Monitor for vibration/movement when armed
  if (antiTheftArmed) {
    // Check for suspicious movement (lower threshold than crash)
    float vibrationLevel = abs(currentTotalAccel - 9.8);  // Subtract gravity
    
    if (vibrationLevel > THEFT_VIBRATION_THRESHOLD) {
      theftDetectionCount++;
      
      Serial.printf("[ANTI-THEFT] ⚠️ Movement detected! Level: %.2f g (Count: %d/%d)\n", 
        vibrationLevel, theftDetectionCount, THEFT_DETECTION_REQUIRED);
      ...
      // Decay detection count if no movement
      if (theftDetectionCount > 0 && millis() % 5000 == 0) {
        theftDetectionCount--;
      }
    }
  }
...
void triggerTheftAlert() {
  ...
  message += "Check your motorcycle immediately!";
...
// ✅ NEW: Log theft event to Firebase
void logTheftToFirebase(String location) {
  ...
  doc["vibrationLevel"] = currentTotalAccel;
